...
select
  ...
  coalesce(
    (
      ...
      where price.pricing_key = 'menu-del-dia-con-bebida'
...
values (
  'menu-vegetariano-del-dia-con-bebida',
  'Menu del dia vegetariano + bebida',
  null,
  null,
  true,
  'menu-vegetariano-del-dia-con-bebida',
...
begin
  ...
  if service_kind not in ('daily-menu', 'grill') then
  ...
  if not exists (
    select 1
    from menu_content.menu_profiles profile
    where profile.id = set_profile_service_kind.profile_id
  ) then
  ...
  where settings.profile_id = set_profile_service_kind.profile_id;

  if current_service_kind = service_kind then
  ...
  values (
    set_profile_service_kind.profile_id,
    set_profile_service_kind.service_kind
  )
